...
# 2. Вставка данных
def insert_data(conn, cursor, patients_data=None, doctors_data=None):
    cursor.execute('DELETE FROM patients')
    cursor.execute('DELETE FROM doctors')
    conn.commit()
    
    # Если данные не переданы — генерируем
    if patients_data is None:
        diagnoses = ['diabetes', 'gastritis', 'schizophrenia', 'hypertension', 'asthma']
        genders = ['female', 'male']
        patients_data = [
            (i, f"Patient {i}", random.choice(diagnoses), random.choice(genders), random.randint(18, 80))
            for i in range(1, 21)
        ]
    ...
    if doctors_data is None:
        ...
        doctors_data = [
            (
                ...
                round(random.choice(base_prices) + random.uniform(-150, 150), 2),
                (start_date + timedelta(days=random.randint(0, 480))).date().isoformat()
            )
            for _ in range(100)
        ]
...
def long_pivot(pivot_table_filled, min_value=500, max_value=10000):
